...
func (vs *ViewServer) tick() {
	vs.mu.Lock()
	defer vs.mu.Unlock()
	//check time of primary/backup & replace	
	primary := vs.view.Primary
	backup  := vs.view.Backup
	if vs.primary_acked{
		if primary != ""{
			timeSinceLastPing := time.Now().Sub(vs.time_map[primary])
			intervals := timeSinceLastPing / PingInterval
			if intervals >= DeadPings{
				vs.view.Primary = ""
			}
		...
		if backup != ""{
			timeSinceLastPing := time.Now().Sub(vs.time_map[backup])
			intervals := timeSinceLastPing / PingInterval
			if intervals >= DeadPings{
				vs.view.Backup = ""
			}
		...
		if vs.view.Primary == "" && vs.view.Backup != ""{
			vs.view.Primary, vs.view.Backup = vs.view.Backup,""
			for key, value := range vs.time_map{
				if key != vs.view.Primary && key != primary{
					timeSinceLastPing := time.Now().Sub(value)
					intervals := timeSinceLastPing / PingInterval
					if intervals <= DeadPings{
						vs.view.Backup = key
			...
			vs.view.Viewnum += 1 
			vs.primary_acked = false
		} else if (vs.view.Primary != "" && vs.view.Backup == "" ) || (vs.view.Primary == "" && vs.view.Backup == ""){
			for key, value := range vs.time_map{
				if key != vs.view.Primary && key != backup{
					timeSinceLastPing := time.Now().Sub(value)
					intervals := timeSinceLastPing / PingInterval
					if intervals <= DeadPings{
						vs.view.Viewnum += 1 
